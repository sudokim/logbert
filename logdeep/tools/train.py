...
class Trainer:
    def __init__(self, model, options):
        ...
        if self.sample == "sliding_window":
            ...
            logkeys, times = split_features(
                self.output_dir + "train", self.train_ratio, scale=None, scale_path=scale_path, min_len=self.min_len
            )
    ...
    def save_checkpoint(self, epoch, save_optimizer=True):
        checkpoint = {
            "epoch": epoch,
        ...
        print(f"Save model checkpoint at {self.model_path}")
    ...
    def train(self, epoch):
        ...
        print(f"\nStarting epoch: {epoch:d} | phase: train | ⏰: {start} | Learning rate: {lr:f}")
        ...
        for i, (log, label) in enumerate(tbar):
            ...
            if (i + 1) % self.accumulation_step == 0:
                self.optimizer.step()
                self.optimizer.zero_grad()
            tbar.set_description(f"Train loss: {total_losses / (i + 1):.5f}")
    ...
    def valid(self, epoch):
        ...
        if total_losses / num_batch < self.best_loss:
            ...
            self.save_checkpoint(epoch, save_optimizer=False)
            self.epochs_no_improve = 0
        else:
